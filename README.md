...
### Что мной сделано в данном проекте:
1. Получение данных с формы заказа
2. Создание моделек для записи данных с формы заказа
3. Связь формы заказа с базой данных и админкой, чтобы оформленный заказ попадал в базу данных
4. Подключение DRF
5. Проверка данных от формы заказа серилайзеры, десирилизация
6. Создание страницы всех необработанных заказов для менеджера http://127.0.0.1/manager/orders/
![скриншот сайта](screenshot/2023-09-04_06-52-16.png)
7. Расчет общей суммы заказа, связь страницы заказа с админкой через кнопку `редактировать`
8. Добавление в интерфейс заказа даты создания, сборки и доставки, способ оплаты
9. Страница с заказами показывает в каком ресторане может приготовиться заказ (исходя из ассортимента ресторанов)
10. Расчет расстояния от адреса клиента до всех ресторанов, способных выполнить заказ, и фильтрация их по данному растоянию от меньшего к большему
11. Создание БД с имеющимися расстояниями, чтобы уменьшить количество запросов к API Яндекса
12. Деплой на арендованном сервере, демонизация
13. Использование Gunicorn, Nginx и связка их
14. Использование логирование через Rollbar
15. Использование Postgres
16. Подключен домен
17. Установлен Certbot с автомотическим обновлением сертификатов
18. Написан bash скрипт для автоматизации деплоя
19. Удаление устаревших сессий по таймеру
...
Перейдите в скаченную папку с проектом на сервере и добавьте переменную окружения
(ROLLBAR_ACCESS_TOKEN можно не добавлять, если вам не нужна информация о деплои в роллбаре):
```sh
ROLLBAR_ACCESS_TOKEN=a50a3b0ab7c44.....
```
А потом запустите скрипт:
```sh
...
### Как собрать бэкенд на локальном компьютере
...
"отладки" в файле `.env` необходимо указать
```sh
